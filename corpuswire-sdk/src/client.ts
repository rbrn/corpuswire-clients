...
export class RemoteIndexDetachedError extends Error {
  ...
  constructor(sessionId: string, status: RemoteIndexStatus, reason: string) {
    super(
      `${reason}; backend indexing continues for session ${sessionId}. `
...
const REVIEW_JOB_STATES_V2: ReadonlySet<unknown> = new Set([
  "queued", "running", "succeeded", "partial", "failed", "cancelled", "superseded",
]);
const SYMBOL_CHANGE_KINDS_V2: ReadonlySet<unknown> = new Set([
...
const PAIRING_STATUSES_V2: ReadonlySet<unknown> = new Set([
  "exact_symbol_id", "exact_analyzer_declaration_id", "exact_unique_declaration_key",
  "one_sided", "ambiguous", "unresolved", "unsupported",
]);
const CONTINUITY_STATUSES_V2: ReadonlySet<unknown> = new Set([
  "proven", "not_established", "ambiguous", "unavailable",
]);
const RELATIONSHIP_DELTA_STATUSES_V2: ReadonlySet<unknown> = new Set([
  "added", "removed", "preserved", "evidence_changed", "ambiguous", "unresolved",
]);
const BUNDLE_OMISSION_REASONS_V2: ReadonlySet<unknown> = new Set([
...
export class CorpusWireClient {
  ...
  async getReviewContextCapabilitiesV2(): Promise<ReviewContextCapabilitiesV2> {
    const result = await requestJson<ReviewContextCapabilitiesV2>({
      baseUrl: this.baseUrl,
  ...
  async requestReviewContextV2(request: ReviewContextRequestV2Input): Promise<ReviewContextResultV2> {
    const codebaseId = requireIdentifier(request.codebaseId, "codebaseId");
    const result = await requestJson<ReviewContextResultV2>({
      baseUrl: this.baseUrl,
  ...
  async getReviewContextJobV2(jobId: string): Promise<ReviewContextResultV2> {
    const result = await requestJson<ReviewContextResultV2>({
      baseUrl: this.baseUrl,
  ...
  async cancelReviewContextJobV2(jobId: string): Promise<ReviewContextJobV2> {
    const result = await requestJson<ReviewContextJobV2>({
      baseUrl: this.baseUrl,
      paths: [`/v2/review-context/jobs/${encodeURIComponent(requireIdentifier(jobId, "jobId"))}`],
      fetchFn: this.fetchFn,
      defaultHeaders: this.defaultHeaders,
  ...
  async pollReviewContextJobV2(
    jobOrId: ReviewContextJobV2 | string,
    options: ReviewContextPollOptionsV2 = {},
  ): Promise<ReviewContextResultV2> {
    const timeoutMs = validateNonNegativeNumber(
      options.timeoutMs ?? DEFAULT_REVIEW_POLL_TIMEOUT_MS,
      "timeoutMs",
    );
    ...
    const deadline = Date.now() + timeoutMs;
    let result: ReviewContextResultV2 = typeof jobOrId === "string"
  ...
  async requestReviewContextV2AndWait(
    request: ReviewContextRequestV2Input,
    options: ReviewContextPollOptionsV2 = {},
  ): Promise<ReviewContextResultV2> {
  ...
  async getReviewStatusV2(codebaseId: string, reviewId: string): Promise<ReviewStatusV2> {
    const result = await requestJson<ReviewStatusV2>({
      baseUrl: this.baseUrl,
      paths: [
  ...
  async previewIndexWorkspace(request: IndexWorkspaceRequest): Promise<RemoteIndexPreview> {
    ...
    if (request.inventoryScan) {
      ...
      if (capabilities.inventory_coverage_versions?.includes(INVENTORY_VERSION)) {
        ...
        const excludedPaths = new Set(
          request.evaluationInventoryAttestation?.excluded_entries.map(
    ...
    const response = await requestJson<{ ok: true; result: RemoteIndexPreview }>({
      baseUrl: this.baseUrl,
  ...
  async indexWorkspace(request: IndexWorkspaceRequest): Promise<RemoteIndexCommitResponse> {
    ...
    try {
      ...
      emitClientProgress(
        ...
        "files",
        "Sending manifest for comparison",
        session.session_id,
      );
      ...
      const expectedEvaluationSkips =
        request.evaluationInventoryAttestation?.excluded_file_count ?? 0;
      if (
        ...
        throw new WorkspaceScanIncompleteError(
          `Server rejected manifest entries${detail}`,
          {
  ...
  private async waitForIndexSessionTerminal(
    sessionId: string,
    pollMs: number,
    onProgress?: (event: RemoteIndexProgressEvent) => void,
    detachSignal?: AbortSignal,
...
export function toReviewContextPayloadV2(
  request: ReviewContextRequestV2Input,
): ReviewContextRequestV2 {
  const budgets = request.budgets === undefined
    ? undefined
  ...
  return removeUndefinedValues({
    schema_version: "review-context/v2" as const,
    codebase_id: request.codebaseId,
    target_repository_id: request.targetRepositoryId,
    provider_review_id: request.providerReviewId,
    expected_head_sha: request.expectedHeadSha,
...
export function isReviewContextJobV2(
  result: ReviewContextResultV2,
): result is ReviewContextJobV2 {
...
function requireReviewContextV2Envelope<T>(
  value: T,
  label: string,
  ...
  if (!isRecord(value) || value.schema_version !== "review-context/v2") {
    throw new Error(
      `Unsupported ${label} schema_version: ${String(
        isRecord(value) ? value.schema_version || "missing" : "missing",
      )}.`,
    );
  }
  if (
    ...
    const contractVersion = "contract_version" in value
      ...
      : "missing";
    throw new Error(
      `Unsupported ${label} contract_version: ${String(contractVersion)}.`,
    );
  }
  return value;
}

function requireReviewContextV2Result(
  value: ReviewContextResultV2,
  label: string,
): ReviewContextResultV2 {
...
function requireReviewContextJobV2(value: unknown, label: string): asserts value is ReviewContextJobV2 {
...
function requireReviewStatusV2(value: unknown, label: string): asserts value is ReviewStatusV2 {
  if (!isReviewStatusEnvelopeV2(value)) {
    throw new Error(`Malformed ${label} v2 status envelope.`);
  }
}

function requireReviewContextResponseV2(
  value: unknown,
  label: string,
): asserts value is ReviewContextResultV2 {
  if (!isRecord(value)) {
...
export function assertReviewContextV2Result(
  value: unknown,
): asserts value is ReviewContextResultV2 {
...
function isReviewBundleV2(value: unknown): boolean {
  if (!hasV2Shape(value, [
  ...
  if (!isReviewSymbolChangeRecordV2(record)) return false;
  for (const [side, instance, evidence] of [
    ...
    if (present !== (evidence !== null && evidence !== undefined)) return false;
    if (!present) continue;
    if (!isSymbolExtentV2(evidence, side)
      || !isRecord(instance)
      || !extentMatchesInstance(evidence, instance, record.change_id, side)) return false;
  }
...
function isReviewSymbolChangeRecordV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, [
  ...
  if ((base !== null && !isSymbolInstanceV2(base)) || (head !== null && !isSymbolInstanceV2(head))) return false;
  if (exact && (!basePresent || !headPresent || value.pairing_confidence !== 1 || value.continuity_status !== "proven")) return false;
  if (exact && ["added", "removed", "ambiguous", "unsupported_split_merge"].includes(String(value.change_kind))) return false;
  if (!exact && value.pairing_confidence !== null) return false;
  if (value.pairing_status === "one_sided" && (sideCount !== 1 || !["added", "removed"].includes(String(value.change_kind)) || value.continuity_status !== "not_established")) return false;
  if (value.pairing_status === "unresolved" && (sideCount !== 1 || value.change_kind !== "unresolved" || value.continuity_status !== "unavailable")) return false;
  if (value.pairing_status === "ambiguous" && (sideCount !== 1 || value.change_kind !== "ambiguous" || value.continuity_status !== "ambiguous")) return false;
  if (value.pairing_status === "unsupported" && (sideCount !== 1 || value.change_kind !== "unsupported_split_merge" || value.continuity_status !== "unavailable")) return false;
  if ((["ambiguous", "unsupported"].includes(String(value.pairing_status))) !== (value.pairing_group !== null)) return false;
  if (value.pairing_group !== null && !isPairingGroupV2(value.pairing_group)) return false;
  if (!isNormalizedDiffEvidenceV2(value.diff_evidence)
    ...
    || !value.relationship_deltas.every(isReviewRelationshipDeltaV2)) return false;
  const diffEvidence = value.diff_evidence;
  const oldPath = diffEvidence.previous_path ?? diffEvidence.path;
  if (value.base_observation.path !== oldPath || value.head_observation.path !== diffEvidence.path) return false;
  if (isRecord(base) && (base.path !== value.base_observation.path || base.layer !== "snapshot")) return false;
  if (isRecord(head) && (head.path !== value.head_observation.path || head.layer !== "overlay")) return false;
  if (isRecord(base) && value.base_observation.source_state === "present"
    && base.source_content_sha256 !== null
    && base.source_content_sha256 !== value.base_observation.source_content_sha256) return false;
  if (isRecord(head) && value.head_observation.source_state === "present"
    && head.source_content_sha256 !== null
    && head.source_content_sha256 !== value.head_observation.source_content_sha256) return false;
  if (isRecord(base) && isRecord(head)
    && (base.revision === head.revision || base.repository_id !== head.repository_id)) return false;
  const roles = {
  ...
  if (!roles || value.base_observation.path_role !== roles[0] || value.head_observation.path_role !== roles[1]) return false;
  if (value.change_kind === "added" && (basePresent || !headPresent)) return false;
  if (value.change_kind === "removed" && (!basePresent || headPresent)) return false;
  if (value.change_kind === "added" && diffEvidence.change_kind === "added"
    && value.base_observation.source_state !== "absent_by_diff") return false;
  if (value.change_kind === "removed" && diffEvidence.change_kind === "deleted"
    && value.head_observation.source_state !== "absent_by_diff") return false;
  if (["unresolved", "ambiguous", "unsupported_split_merge"].includes(String(value.change_kind))
    && value.completeness.symbol_pair_complete) return false;
  ...
  if (!isSortedUniqueNumbers(hunkOrdinals)) return false;
  if (value.normalized_hunks.some((hunk) => hunk.path !== diffEvidence.path)) return false;
  if (["missing", "not_applicable"].includes(String(diffEvidence.patch_status)) && value.normalized_hunks.length > 0) return false;
  if (diffEvidence.patch_status !== "complete" && value.completeness.normalized_hunks_complete) return false;
  if (diffEvidence.patch_status === "complete" && value.normalized_hunks.some((hunk) => (
    hunk.lines.filter((line: Record<string, unknown>) => line.old_line !== null).length !== hunk.old_count
    || hunk.lines.filter((line: Record<string, unknown>) => line.new_line !== null).length !== hunk.new_count
  ))) return false;
  if (value.relationship_deltas.some((delta) => delta.changed_logical_identity !== value.logical_identity)) return false;
  for (const delta of value.relationship_deltas) {
    for (const [fact, instance, expectedLayer] of [
      ...
      if (!isRecord(instance)
        || !relationshipFactIsIncidentToInstance(delta, fact, instance, expectedLayer)) return false;
    }
  }
  return isNonEmptyId(value.change_id) && isNonEmptyId(value.logical_identity);
}

function isReviewRelationshipDeltaV2(value: unknown): boolean {
  if (!hasV2Shape(value, [
  ...
  if (!(["incoming", "outgoing"] as const).includes(value.direction as "incoming" | "outgoing")) {
  ...
  if (!basePresent && !headPresent) return false;
  if (value.status === "added" && (basePresent || !headPresent)) return false;
  if (value.status === "removed" && (!basePresent || headPresent)) return false;
  if (["preserved", "evidence_changed"].includes(String(value.status)) && (!basePresent || !headPresent)) return false;
  if ((value.status === "evidence_changed") !== (Array.isArray(value.comparison_attributes_changed) && value.comparison_attributes_changed.length > 0)) return false;
  if (!isSortedUniqueStrings(value.comparison_attributes_changed as string[])) return false;
  ...
  if (definitive && (!isRecord(completeness)
    ...
    || (value.direction === "incoming" && completeness.incoming_dependents_reanalyzed !== true))) return false;
  for (const fact of [value.base_fact, value.head_fact]) {
    if (!isRecord(fact)) continue;
    if (fact.fact_type === "resolved" && fact.endpoint_logical_identity !== value.endpoint_logical_identity) return false;
    if (fact.fact_type === "unresolved" && value.endpoint_logical_identity !== null) return false;
  }
  const missingContractSemantics = [value.base_fact, value.head_fact].some((fact) => (
    isRecord(fact) && fact.fact_type === "resolved" && fact.contract_evidence_id !== null
  ...
  if (missingContractSemantics && (value.status !== "unresolved"
    ...
    || !completeness.reason_codes.includes("contract_semantic_evidence_unavailable"))) return false;
  return factsValid
...
function isReviewOmissionV2(value: unknown): boolean {
  if (!hasV2Shape(value, [
...
function isMinimumReviewBudgetV2(value: unknown): boolean {
...
function isReviewPartialReasonV2(value: unknown): boolean {
...
function isReviewResponseEnvelopeV2(value: Record<string, unknown>): boolean {
  if (!hasV2Shape(value, [
  ...
  if (!isReviewScopeV2(scope)
    ...
    || !Array.isArray(value.omitted_bundles) || !value.omitted_bundles.every(isReviewOmissionV2)) return false;
  if (value.target_repository_id !== scope.target_repository_id) return false;
  ...
  if (publication.some((item, index) => item !== scoped[index])) return false;
  const admittedOrdinals = value.bundles
    .map((item) => (item as Record<string, unknown>).ordinal as number);
  ...
  if (!isSortedNumbers(admittedOrdinals) || !isSortedNumbers(omittedOrdinals)
    ...
    || [...ordinals].sort((left, right) => left - right)
      .some((ordinal, index) => ordinal !== index)) return false;
  const changeIds = [...value.bundles.map((bundle) => (bundle as Record<string, unknown>).change_record)
    .map((record) => (record as Record<string, unknown>).change_id),
  ...value.omitted_bundles.map((item) => (item as Record<string, unknown>).change_id)];
  if (new Set(changeIds).size !== changeIds.length) return false;
  if ((value.freshness === "partial") !== value.partial
    || value.partial !== (value.partial_reasons.length > 0 || value.omitted_bundles.length > 0)) return false;
  return value.bundles.every((bundle) => {
    const record = (bundle as Record<string, unknown>).change_record as Record<string, unknown>;
    if ((record.diff_evidence as Record<string, unknown>).normalized_diff_hash !== value.normalized_diff_hash) return false;
    for (const [instance, revision, layer] of [[record.base, value.base_sha, "snapshot"], [record.head, value.head_sha, "overlay"]]) {
      ...
      if (!isRecord(instance) || instance.repository_id !== value.target_repository_id
        || instance.revision !== revision || instance.layer !== layer) return false;
    }
    const authorized = scope.authorized_repository_ids as string[];
    if (!(bundle as Record<string, unknown>).related_evidence
      || !((bundle as Record<string, unknown>).related_evidence as unknown[])
        .every((evidence) => isRecord(evidence) && authorized.includes(String(evidence.repository_id)))) return false;
    for (const delta of record.relationship_deltas as Record<string, unknown>[]) {
      for (const [fact, expectedGeneration] of [
        ...
        if (!isRecord(fact)) continue;
        if (!relationshipFactMatchesPublication(
          fact, String(value.base_snapshot_id), Number(expectedGeneration),
        )) return false;
        if (fact.fact_type === "resolved"
          && (!authorized.includes(String(fact.source_repository_id))
            || !authorized.includes(String(fact.target_repository_id)))) return false;
      }
    }
    return true;
  });
...
function isReviewContextJobEnvelopeV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, [
  ...
  ]) || value.contract_version !== "review-context/v2") return false;
  if (![value.job_id, value.request_id, value.tenant_id, value.codebase_id,
    ...
    || !Array.isArray(value.partial_reasons) || !value.partial_reasons.every(isReviewPartialReasonV2)) return false;
  const pinned = [value.overlay_id, value.overlay_generation, value.overlay_refresh_sequence];
  if (pinned.some((item) => item === null) !== pinned.every((item) => item === null)) return false;
  const completed = value.state === "succeeded" || value.state === "partial";
  if (completed !== pinned.every((item) => item !== null)) return false;
  const pending = value.state === "queued" || value.state === "running";
  if (pending !== (value.retry_after_seconds !== null)) return false;
  if (value.retry_after_seconds !== null
    && (!isNonNegativeInteger(value.retry_after_seconds) || Number(value.retry_after_seconds) > 3600)) return false;
  if (Date.parse(String(value.updated_at)) < Date.parse(String(value.created_at))) return false;
  const reasonKeys = value.partial_reasons.map((reason) => (
    `${reason.code}\0${reason.affected_side}\0${String(reason.retryable)}`
  ));
  if (!isSortedUniqueStrings(reasonKeys)) return false;
  if (value.state === "succeeded" && value.partial_reasons.length > 0) return false;
  if (value.state === "partial" && value.partial_reasons.length === 0) return false;
  return true;
}

function isReviewStatusEnvelopeV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["contract_version", "codebase_id", "review_id", "target_repository_id", "head_sha", "publications", "latest_jobs"])
    ...
    || !Array.isArray(value.latest_jobs) || !value.latest_jobs.every(isReviewContextJobEnvelopeV2)) return false;
...
function isReviewPublicationV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["repository_set_id", "repository_selection_digest", "target_repository_id", "base_sha", "head_sha", "base_snapshot_id", "base_snapshot_generation", "base_snapshot_refresh_sequence", "overlay_id", "overlay_generation", "overlay_refresh_sequence", "state", "freshness", "evidence_state", "warning_count", "published_at"])) return false;
...
function isReviewCapabilitiesV2(value: unknown): boolean {
...
function isReviewScopeV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["tenant_id", "actor_id", "codebase_id", "target_repository_id", "authorized_repository_ids", "repository_set_id", "repository_selection_digest", "base_snapshot_id", "base_snapshot_generation", "base_snapshot_refresh_sequence", "overlay_id", "overlay_generation", "overlay_refresh_sequence"])) return false;
...
function isSymbolInstanceV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["symbol_id", "symbol_instance_id", "repository_id", "revision", "layer", "path", "source_range", "language", "project_root", "qualified_name", "display_name", "kind", "signature", "source_content_sha256", "symbol_extent_sha256", "stable_declaration_identity", "provenance"])) return false;
...
function isNormalizedDiffEvidenceV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["normalized_diff_hash", "path", "previous_path", "change_kind", "content_kind", "patch_status", "additions", "deletions"])) return false;
...
function isPathObservationV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["path", "path_role", "source_state", "analyzer_state", "analyzed_scope_complete", "source_content_sha256", "reason_codes"])) return false;
...
function isPathObservationTruthful(value: Record<string, unknown>): boolean {
  ...
  if (value.source_content_sha256 !== null || value.analyzed_scope_complete) return false;
  if (value.source_state === "non_text") return value.analyzer_state === "not_applicable";
  return value.source_state === "unavailable"
    && (value.analyzer_state === "failed" || value.analyzer_state === "unavailable");
}

function isChangeCompletenessV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["symbol_pair_complete", "normalized_hunks_complete", "relationship_deltas_complete", "complete", "reason_codes"])) return false;
...
function isPairingGroupV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["group_id", "tier", "base_candidate_instance_ids", "head_candidate_instance_ids", "base_candidate_count", "head_candidate_count", "candidates_truncated"])) return false;
  return isNonEmptyId(value.group_id)
    ...
    && value.candidates_truncated === (
      Number(value.base_candidate_count) > value.base_candidate_instance_ids.length
      || Number(value.head_candidate_count) > value.head_candidate_instance_ids.length
    );
}

function isNormalizedHunkV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["path", "ordinal", "old_start", "old_count", "new_start", "new_count", "section", "lines", "hunk_sha256"])) return false;
...
function isNormalizedDiffLineV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["kind", "text", "old_line", "new_line", "no_newline_at_end"])) return false;
...
function isRelationshipFactV2(value: Record<string, unknown>): boolean {
  ...
  if (!hasV2Shape(value, ["fact_type", "reference_id", "direction", "relationship_kind", "changed_logical_identity", "normalized_target", "source_symbol_id", "source_symbol_instance_id", "path", "source_range", "provenance", "resolution_status", "candidate_count", "reason", "revision", "layer", "generation"])) return false;
...
function isSymbolExtentV2(value: unknown, side: "base" | "head"): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["side", "evidence_id", "symbol_instance_id", "repository_id", "revision", "layer", "path", "symbol_source_range", "extent_start_line", "extent_end_line", "source_content_sha256", "symbol_extent_sha256", "text", "token_count"])) return false;
...
function extentMatchesInstance(extent: Record<string, unknown>, instance: Record<string, unknown>, changeId: unknown, side: "base" | "head"): boolean {
  if (!isNonEmptyId(changeId)) return false;
...
function canonicalExtentEvidenceId(changeId: string, side: string, instance: Record<string, unknown>): string {
  const range = instance.source_range as Record<string, unknown>;
...
function isEvidenceItemV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["evidence_id", "repository_id", "revision", "layer", "path", "source_range", "content_hash", "text", "symbol_id", "graph_distance", "provenance", "freshness", "confidence", "selection_reason", "token_count"])) return false;
...
function isBundleCompletenessV2(value: unknown): value is Record<string, unknown> {
...
function isProvenanceV2(value: unknown): boolean {
  if (!hasV2Shape(value, ["extractor_id", "extractor_version", "evidence_tier", "resolution_status", "confidence", "reason_codes", "artifact_digest"])) return false;
...
function isStableDeclarationV2(value: unknown): value is Record<string, unknown> {
...
function isSourceRangeV2(value: unknown): value is Record<string, unknown> {
  if (!hasV2Shape(value, ["start_line", "end_line", "start_column", "end_column"])) return false;
...
function isAuthoritativeReviewObservation(value: Record<string, unknown>): boolean {
  return value.source_state === "absent_by_diff" || (
    value.source_state === "present"
...
function relationshipFactIsIncidentToInstance(
  delta: Record<string, unknown>,
  fact: Record<string, unknown>,
  instance: Record<string, unknown>,
  expectedLayer: "snapshot" | "overlay",
): boolean {
  if (fact.layer !== expectedLayer || fact.revision !== instance.revision) return false;
  ...
  if (incident.some((item, index) => item !== expected[index])) return false;
  return delta.direction !== "outgoing" || (
    fact.path === instance.path && sourceRangeContains(instance.source_range, fact.source_range)
  );
}

function relationshipFactMatchesPublication(
  fact: Record<string, unknown>,
  baseSnapshotId: string,
  expectedGeneration: number,
): boolean {
  if (fact.fact_type === "unresolved") return fact.generation === expectedGeneration;
...
function sourceRangeContains(outer: unknown, inner: unknown): boolean {
  if (!isSourceRangeV2(outer) || !isSourceRangeV2(inner)) return false;
  if (Number(inner.start_line) < Number(outer.start_line)
    || Number(inner.end_line) > Number(outer.end_line)) return false;
  if (inner.start_line === outer.start_line && outer.start_column !== null
    && inner.start_column !== null
    && Number(inner.start_column) < Number(outer.start_column)) return false;
  if (inner.end_line === outer.end_line && outer.end_column !== null
    && inner.end_column !== null
    && Number(inner.end_column) > Number(outer.end_column)) return false;
  return true;
}

function sourceRangesEqual(left: unknown, right: unknown): boolean {
  if (!isSourceRangeV2(left) || !isSourceRangeV2(right)) return false;
...
function hasV2Shape(value: unknown, required: readonly string[]): value is Record<string, unknown> {
  if (!isRecord(value) || value.schema_version !== "review-context/v2"
...
function isSortedNumbers(values: readonly number[]): boolean {
  return values.every((value, index) => index === 0 || values[index - 1] <= value);
}
...
function isIsoDate(value: unknown): boolean {
  ...
  if (datePart === undefined || timeAndZone === undefined) return false;
...
function isCanonicalPath(value: unknown, allowRoot = false): value is string {
  if (typeof value !== "string" || value.length > 4096 || value.includes("\\") || value.startsWith("/") || value.includes("\0")) return false;
